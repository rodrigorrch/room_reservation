...
$(document).ready(function () {
  ...
  var requestWeek = function requestWeek(week) {
    $.ajax({
      method: "get",
      url: "/api/reunion",
      data: {
  ...
  var saveReunion = function saveReunion(data, reference) {
    $.ajax({
      method: "POST",
  ...
  var removeReunion = function removeReunion(datas, reference) {
    $.ajax({
      method: "DELETE",
  ...
  var loadWeekByDate = function loadWeekByDate(date) {
    var monday = moment(date).startOf('isoWeek');
    var listWeek = [monday.clone().toDate(), monday.clone().add(1, "days").toDate(), monday.clone().add(2, "days").toDate(), monday.clone().add(3, "days").toDate(), monday.clone().add(4, "days").toDate(), monday.clone().add(5, "days").toDate(), monday.clone().add(6, "days").toDate()];
  ...
  $('#roomModal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this);
    var id = link.data('id');
    var date = _typeof(link.data('week')) == 'object' ? link.data('week').date : link.data('week');
...
module.exports = __webpack_require__(/*! C:\wamp64\www\reunion\resources\js\home.js */"./resources/js/home.js");
